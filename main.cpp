...
const char* PROGRAM_TITLE = "tinyrender";
...
public:
    void run() {
        std::cerr << "Initializing app" << std::endl;
        initWindow();
        initVulkan();
        mainLoop();
        cleanup();
...
private:
    ...
    // Graphics Pipiline related members
    ...
    void initWindow() {
        ...
        window = glfwCreateWindow(WIDTH, HEIGHT, PROGRAM_TITLE, nullptr, nullptr);
    ...
    void createInstance() {
        ...
        /*
            This data is technically optional, but it may provide some useful information to the
        ...
        */
        ...
        appInfo.pApplicationName = PROGRAM_TITLE;
        appInfo.applicationVersion = VK_MAKE_VERSION(1, 0, 0);
        appInfo.pEngineName = PROGRAM_TITLE;
    ...
    void createGraphicsPipeline() {
        auto vertShaderCode = readFile("src/shaders/compiled/shader.vert.spv");
        auto fragShaderCode = readFile("src/shaders/compiled/shader.frag.spv");
        ...
        // Takes the geometry that is shaped by the vertices from the vertex shader and
    ...
    void createCommandPool() {
        /*
            Command pools manage the memory that is used to store the buffers and command buffers
    ...
    void createTextureImage() {
        ...
        */
        ...
        // Buffer should be in host visible memory so that we can map it and it should be usable as a transfer source 
        createBuffer(
            imageSize,
    ...
    void createTextureImageView() {
        // Texture images are accessed through image views rather than directly
    ...
    void createTextureSampler() {
        /*
            ...
            See: https://vulkan-tutorial.com/images/texture_filtering.png
    ...
    void drawFrame() {
        /*
            ...
            Each of these events are executed asynchronously but each of the operations depends on the previous one finishing.
    ...
    void populateDebugMessengerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT& createInfo) {
        /*
            There are many more ways to configure validation layer messages and debug callbacks.
             See the extension specification (link) for more info about the possibilities.
    ...
    int rateDeviceSuitability(VkPhysicalDevice device) {
        ...
        // Query for basic device properties like the name, type and supported Vulkan version
        vkGetPhysicalDeviceProperties(device, &deviceProperties);
        // Query for optional features like texture compression, 64 bit floats and multi viewport rendering
        vkGetPhysicalDeviceFeatures(device, &deviceFeatures);

        QueueFamilyIndices indices = findQueueFamilies(device);

        bool supportsFeatures = deviceFeatures.samplerAnisotropy && deviceFeatures.geometryShader;
        ...
        if (!supportsFeatures || !indices.isComplete() || !extensionsSupported || !swapChainAdequate) {
            return 0;
        }
    ...
    VkSurfaceFormatKHR chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
        ...
        for (const auto& availableFormat : availableFormats) {
            // VK_FORMAT_B8G8R8A8_SRGB means that we store the B, G, R and alpha channels in that order
    ...
    VkPresentModeKHR chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) {
        /*
            ...
            VK_PRESENT_MODE_FIFO_KHR:
                The swap chain is a queue where the display takes an image from the front of the queue when the display is refreshed
    ...
    VkExtent2D chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
        ...
        // Some window managers allow us to differ here and this is indicated by setting the width and height in currentExtent to UINT32_MAX
    ...
    uint32_t findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
        ...
        uint32_t i = 0;
        while (i < memProperties.memoryTypeCount && !(typeFilter & (1 << i) && (memProperties.memoryTypes[i].propertyFlags & properties) == properties)) {
            i++;
        }
